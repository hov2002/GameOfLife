...
function addPredator() {
    ...
    var y = Math.floor(Math.random() * matrix.length)
        if (matrix[y][x] == 0) {
            matrix[y][x] = 3
            predatorArr.push(new GrassEater(x, y, 3))
        }
    }
    io.sockets.emit("send matrix", matrix);
}
...
function addCavalier(){
    ...
    var y = Math.floor(Math.random() * matrix.length)
        if (matrix[y][x] == 0) {
            matrix[y][x] = 4
            predatorArr.push(new GrassEater(x, y, 4))
        }
    }
    io.sockets.emit("send matrix", matrix);
}
function addFly(){
    ...
    var y = Math.floor(Math.random() * matrix.length)
        if (matrix[y][x] == 0) {
            matrix[y][x] = 5
            predatorArr.push(new GrassEater(x, y, 5))
        }
    }
    io.sockets.emit("send matrix", matrix);
}
...
io.on('connection', function (socket) {
    ...
    socket.on("add cavalier", addCavalier);
    socket.on("add fly", addFly);
});
